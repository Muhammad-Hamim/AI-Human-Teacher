...
const processMessage = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!message || !message.chatId) {
      return next(
        new AppError(httpStatus.BAD_REQUEST, "Missing chatId in message")
      );
    }

    // Handle missing message.message.content by checking if it exists or adding it
    if (!message.message || !message.message.content) {
      // If there's a top-level content property, use it to create the required structure
      if (message.content) {
        message.message = {
          content: message.content,
          contentType: message.message?.contentType || "text",
        };
      } else {
        return next(
